...
class poseDetector():
    ...
    def pose3D(self, img, draw=True, display=True):
        ...
        for landmark in self.results.pose_landmarks.landmark: 
            landmarks.append((int(landmark.x * image_width), int(landmark.y * image_height),
    ...
    def poseClassify(self, landmarks, output_image, draw=True, display=True):
        ...
        if label != 'UnknownPose':      
            color = (0, 255, 0)          
        
        cv2.putText(output_image, label, (10, 45),cv2.FONT_HERSHEY_PLAIN, 4, color, 4)           
